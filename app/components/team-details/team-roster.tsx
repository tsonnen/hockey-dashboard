...
function RosterTable({
  ...
  players: Player[];
  columns: { key: keyof Player; label: string; format?: (val: number) => string }[];
  title: string;
}) {
    ...
    <div className="overflow-x-auto">
      ...
      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        <thead className="bg-gray-50 dark:bg-gray-900">
          <tr>
            <th className="px-3 py-2 text-left text-xs font-medium uppercase tracking-wider text-gray-500 underline decoration-dotted">
              #
            </th>
        ...
        <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
          {players.map((p) => (
            <tr
              key={`${p.id || `${p.firstName.default}-${p.lastName.default}-${p.sweaterNumber}`}`}
            ...
            >
              ...
              {visibleColumns.map((col) => (
                <td
                  key={col.key.toString()}
                  className="whitespace-nowrap px-3 py-2 text-right text-sm text-gray-500 dark:text-gray-300"
                >
                  {col.format && typeof p[col.key] === 'number'
                    ? col.format(p[col.key] as number) // This will be a number in this case
                    : (p[col.key]?.toString() ?? '-')}
                </td>
              ))}
            </tr>
...
export function TeamRoster({ roster }: TeamRosterProps) {
  ...
  const skaterColumns: { key: keyof Player; label: string; format?: (val: number) => string }[] = [
  ...
  const goalieColumns: { key: keyof Player; label: string; format?: (val: number) => string }[] = [
    { key: 'gamesPlayed', label: 'GP' },
    { key: 'wins', label: 'W' },
    { key: 'losses', label: 'L' },
  ...
  return (
    <div className="space-y-12">
      <RosterTable players={skaters} columns={skaterColumns} title="Skaters" />
      <RosterTable players={roster.goalies} columns={goalieColumns} title="Goalies" />
    </div>
  );
}
